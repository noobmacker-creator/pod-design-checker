...
import BatchDownloadsPanel from './components/BatchDownloadsPanel';
import ColorCheckWorkspace from './components/ColorCheckWorkspace';
import ColorCheckPanel from './components/ColorCheckPanel';
import PrintSizeCalculatorWorkspace from './components/PrintSizeCalculatorWorkspace';
import PrintSizeGuidePanel from './components/PrintSizeGuidePanel';
import FileInspectorWorkspace from './components/FileInspectorWorkspace';
import FileInfoGuidePanel from './components/FileInfoGuidePanel';
import FilenameCleanerWorkspace, {
  type FilenameCleanerFileEntry,
} from './components/FilenameCleanerWorkspace';
import FilenameGuidePanel from './components/FilenameGuidePanel';
import ListingImageCheckerWorkspace from './components/ListingImageCheckerWorkspace';
import ListingImageGuidePanel from './components/ListingImageGuidePanel';
...
export default function Page() {
  ...
  const [toolsTab, setToolsTab] = useState<'export' | 'batch' | 'converter' | 'colorcheck' | 'printsize' | 'fileinspector' | 'filenamecleaner' | 'listingcheck'>('export');
  const [batchQueue, setBatchQueue] = useState<BatchQueueItem[]>([]);
  const [colorCheckFile, setColorCheckFile] = useState<File | null>(null);
  const [colorCheckImg, setColorCheckImg] = useState<HTMLImageElement | null>(null);
  const [colorCheckFileUrl, setColorCheckFileUrl] = useState<string | null>(null);
  const [colorCheckUploadKey, setColorCheckUploadKey] = useState(0);
  const [colorCheckSelectedPresets, setColorCheckSelectedPresets] = useState<Set<string>>(
    () => new Set(DEFAULT_SELECTED_PRESET_IDS),
  );
  const [colorCheckCustomColours, setColorCheckCustomColours] = useState<string[]>([]);
  const [colorCheckShowCheckerboard, setColorCheckShowCheckerboard] = useState(false);
  const [printSizeFile, setPrintSizeFile] = useState<File | null>(null);
  const [printSizeImg, setPrintSizeImg] = useState<HTMLImageElement | null>(null);
  const [printSizeFileUrl, setPrintSizeFileUrl] = useState<string | null>(null);
  const [printSizeUploadKey, setPrintSizeUploadKey] = useState(0);
  const [fileInspectorFile, setFileInspectorFile] = useState<File | null>(null);
  const [fileInspectorImg, setFileInspectorImg] = useState<HTMLImageElement | null>(null);
  const [fileInspectorFileUrl, setFileInspectorFileUrl] = useState<string | null>(null);
  const [fileInspectorUploadKey, setFileInspectorUploadKey] = useState(0);
  const [fileInspectorDpi, setFileInspectorDpi] = useState<number | null>(null);
  const [filenameCleanerEntries, setFilenameCleanerEntries] = useState<FilenameCleanerFileEntry[]>([]);
  const [filenameCleanerUploadKey, setFilenameCleanerUploadKey] = useState(0);
  const [listingCheckFile, setListingCheckFile] = useState<File | null>(null);
  const [listingCheckImg, setListingCheckImg] = useState<HTMLImageElement | null>(null);
  const [listingCheckFileUrl, setListingCheckFileUrl] = useState<string | null>(null);
  const [listingCheckUploadKey, setListingCheckUploadKey] = useState(0);
  const [listingCheckShowSafeArea, setListingCheckShowSafeArea] = useState(true);
  const [batchDownloadsFocusToken, setBatchDownloadsFocusToken] = useState(0);
  const [batchReviewRequestToken, setBatchReviewRequestToken] = useState(0);
  ...
  useEffect(() => {
    return () => {
      if (colorCheckFileUrl) URL.revokeObjectURL(colorCheckFileUrl);
    };
  }, [colorCheckFileUrl]);

  useEffect(() => {
    return () => {
      if (printSizeFileUrl) URL.revokeObjectURL(printSizeFileUrl);
    };
  }, [printSizeFileUrl]);

  useEffect(() => {
    return () => {
      if (fileInspectorFileUrl) URL.revokeObjectURL(fileInspectorFileUrl);
    };
  }, [fileInspectorFileUrl]);

  useEffect(() => {
    return () => {
      if (listingCheckFileUrl) URL.revokeObjectURL(listingCheckFileUrl);
    };
  }, [listingCheckFileUrl]);

  function handleColorCheckAddDesign(selectedFile: File) {
    if (colorCheckFileUrl) URL.revokeObjectURL(colorCheckFileUrl);
    const url = URL.createObjectURL(selectedFile);
    const image = new Image();
    image.onload = () => {
      setColorCheckFile(selectedFile);
      setColorCheckImg(image);
      setColorCheckFileUrl(url);
    };
    image.onerror = () => {
  ...
  function handleColorCheckClear() {
    if (colorCheckFileUrl) URL.revokeObjectURL(colorCheckFileUrl);
    setColorCheckFile(null);
    setColorCheckImg(null);
    setColorCheckFileUrl(null);
    setColorCheckUploadKey((value) => value + 1);
  }

  function handlePrintSizeAddDesign(selectedFile: File) {
    if (printSizeFileUrl) URL.revokeObjectURL(printSizeFileUrl);
    const url = URL.createObjectURL(selectedFile);
    const image = new Image();
    image.onload = () => {
      setPrintSizeFile(selectedFile);
      setPrintSizeImg(image);
      setPrintSizeFileUrl(url);
    };
    image.onerror = () => {
      URL.revokeObjectURL(url);
    };
  ...
  function handlePrintSizeClear() {
    if (printSizeFileUrl) URL.revokeObjectURL(printSizeFileUrl);
    setPrintSizeFile(null);
    setPrintSizeImg(null);
    setPrintSizeFileUrl(null);
    setPrintSizeUploadKey((value) => value + 1);
  }

  async function handleFileInspectorAddDesign(selectedFile: File) {
    if (fileInspectorFileUrl) URL.revokeObjectURL(fileInspectorFileUrl);
    const url = URL.createObjectURL(selectedFile);
    const image = new Image();
    image.onload = async () => {
      setFileInspectorFile(selectedFile);
      setFileInspectorImg(image);
      setFileInspectorFileUrl(url);
      try {
        const arrayBuffer = await selectedFile.arrayBuffer();
        setFileInspectorDpi(getImageDpi(selectedFile, arrayBuffer));
      } catch {
        setFileInspectorDpi(null);
      }
    };
    image.onerror = () => {
      URL.revokeObjectURL(url);
  ...
  function handleFileInspectorClear() {
    if (fileInspectorFileUrl) URL.revokeObjectURL(fileInspectorFileUrl);
    setFileInspectorFile(null);
    setFileInspectorImg(null);
    setFileInspectorFileUrl(null);
    setFileInspectorDpi(null);
    setFileInspectorUploadKey((value) => value + 1);
  }

  function handleListingCheckAddImage(selectedFile: File) {
    if (listingCheckFileUrl) URL.revokeObjectURL(listingCheckFileUrl);
    const url = URL.createObjectURL(selectedFile);
    const image = new Image();
    image.onload = () => {
      setListingCheckFile(selectedFile);
      setListingCheckImg(image);
      setListingCheckFileUrl(url);
    };
    image.onerror = () => {
      URL.revokeObjectURL(url);
    };
  ...
  function handleListingCheckClear() {
    if (listingCheckFileUrl) URL.revokeObjectURL(listingCheckFileUrl);
    setListingCheckFile(null);
    setListingCheckImg(null);
    setListingCheckFileUrl(null);
    setListingCheckUploadKey((value) => value + 1);
  }

  const colorCheckActiveColours = useMemo(
    () => buildActiveColourEntries(colorCheckSelectedPresets, colorCheckCustomColours),
    [colorCheckSelectedPresets, colorCheckCustomColours],
  );
...
`;
  ...
  function handleOpenBatchDownloads() {
    setBatchDownloadsFocusToken((value) => value + 1);
  }

  function handleBatchReviewFromDownloads() {
    setBatchReviewRequestToken((value) => value + 1);
  }
...
<div className="pod-grid-child-center">
          {toolsTab === 'converter' ? (
            ...
            <ColorCheckWorkspace
              file={colorCheckFile}
              img={colorCheckImg}
              activeColours={colorCheckActiveColours}
              showCheckerboard={colorCheckShowCheckerboard}
              onAddDesign={handleColorCheckAddDesign}
              onClear={handleColorCheckClear}
              uploadInputKey={colorCheckUploadKey}
            />
          ) : toolsTab === 'printsize' ? (
            <PrintSizeCalculatorWorkspace
              file={printSizeFile}
              img={printSizeImg}
              onAddDesign={handlePrintSizeAddDesign}
              onClear={handlePrintSizeClear}
              uploadInputKey={printSizeUploadKey}
            />
          ) : toolsTab === 'fileinspector' ? (
            <FileInspectorWorkspace
              file={fileInspectorFile}
              img={fileInspectorImg}
              dpiMetadata={fileInspectorDpi}
              onAddDesign={handleFileInspectorAddDesign}
              onClear={handleFileInspectorClear}
              uploadInputKey={fileInspectorUploadKey}
            />
          ) : toolsTab === 'filenamecleaner' ? (
            <FilenameCleanerWorkspace
              entries={filenameCleanerEntries}
              onEntriesChange={setFilenameCleanerEntries}
              uploadInputKey={filenameCleanerUploadKey}
              onDownloadMessage={setDownloadMessage}
            />
          ) : toolsTab === 'listingcheck' ? (
            <ListingImageCheckerWorkspace
              file={listingCheckFile}
              img={listingCheckImg}
              showSafeArea={listingCheckShowSafeArea}
              onShowSafeAreaChange={setListingCheckShowSafeArea}
              onAddImage={handleListingCheckAddImage}
              onClear={handleListingCheckClear}
              uploadInputKey={listingCheckUploadKey}
            />
          ) : toolsTab === 'batch' ? (
            <BatchWorkspace
              items={batchQueue}
              onItemsChange={setBatchQueue}
              onOpenInChecker={handleLoadFileFromBatch}
              onOpenDownloads={handleOpenBatchDownloads}
              reviewRequestToken={batchReviewRequestToken}
              uploadNotesProps={{
                file,
                img,
                imgW,
                ...
                autoFixApplied: hasAutoFixApplied,
                downloadMessage,
                displayScore: displayPrintScore,
                scanStatus: img ? 'Scanned' : 'No design',
              }}
            />
          ) : (
          <DesignPreviewPanel
...
{toolsTab === 'batch' ? (
  <BatchDownloadsPanel
    ...
    focusToken={batchDownloadsFocusToken}
    onReviewResults={handleBatchReviewFromDownloads}
  />
) : toolsTab === 'colorcheck' ? (
  <ColorCheckPanel
    file={colorCheckFile}
    img={colorCheckImg}
    selectedPresetIds={colorCheckSelectedPresets}
    onSelectedPresetIdsChange={setColorCheckSelectedPresets}
    customColours={colorCheckCustomColours}
    onCustomColoursChange={setColorCheckCustomColours}
    showCheckerboard={colorCheckShowCheckerboard}
    onShowCheckerboardChange={setColorCheckShowCheckerboard}
  />
) : toolsTab === 'printsize' ? (
  <PrintSizeGuidePanel />
) : toolsTab === 'fileinspector' ? (
